...
/** Origin (scheme + host) the API lives on — used for non-client HTTP calls. */
...
/** Admin-only: read back the full OAuth settings, including the server-computed Redirect URIs. */
export async function getOAuthAdminSettings(): Promise<{ data: OAuthAdminSettings | null; error: { message: string } | null }> {
  return http('/oauth/settings')
}
  ...
  auth: {
    ...
    async signUp({ email, password, options }: { email: string; password: string; options?: { data?: Record<string, string> } }) {
      ...
      const body: Record<string, unknown> = {
        ...
        roleSlug: meta.role_slug ?? meta.roleSlug ?? undefined,
        registrationToken: meta.registration_token ?? meta.registrationToken ?? undefined,
      }
    ...
    /** Store a session minted by the Microsoft OAuth exchange (login or signup). */
    async setOAuthSession({ accessToken, refreshToken, user }: { accessToken: string; refreshToken: string; user: Record<string, any> }) {
      const u = toUser(user)
      const aal: 'aal1' | 'aal2' = user?.mfaEnabled ? 'aal1' : 'aal2'
      const session: StoredSession = {
      ...
      saveSession(session)
      emitAuth('SIGNED_IN', toStoredSession(session))
      return { data: { user: u, session: toStoredSession(session) }, error: null }
    },
